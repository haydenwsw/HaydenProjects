...
namespace AutoHarvest.Scrapers
{
    ...
    public partial class FbMarketplace
    {
        ...
        public FbMarketplace(IHttpClientFactory httpclientfactory, IOptions<CarFinder> carfinder, ILogger<FbMarketplace> logger)
        {
            ...
            if (File.Exists(CarFinder.GetFacebookCookiesPath))
            {
                string[] cookies = File.ReadAllLines(CarFinder.GetFacebookCookiesPath);
                Cookie = cookies[0];
                X_ASBD_ID = cookies[1];
            }
            else
                throw new Exception("The website cannot startup, missing Facebook Marketplace cookies");
            
            // get the graph ql keys for scraping
            if (File.Exists(CarFinder.GetMarketplaceKeysPath))
            {
                string json = File.ReadAllText(CarFinder.GetMarketplaceKeysPath);
            ...
            else
            {
                ...
                File.WriteAllText(CarFinder.GetMarketplaceKeysPath, json);
            }
        }

        public async Task<List<Car>> ScrapeCars(FilterOptions filterOptions, CarLookup carLookup)
        {
            FbMarketplaceVariables fbMarketplaceVariables = null;

            try
            {
                // if there is no keys regenerate them
                if (!File.Exists(CarFinder.GetMarketplaceKeysPath))
                {
                    MarketplaceRequest = GetFbMarketplaceKeys().Result;
                    string jsonKeys = JsonConvert.SerializeObject(MarketplaceRequest, Formatting.Indented);
                    File.WriteAllText(CarFinder.GetMarketplaceKeysPath, jsonKeys);
                }
                ...
                if (edges.Length == 0)
                {
                    Logger.LogInformation("FbMarketplace scraper the listings are empty. fbMarketplaceVariables: {fbMarketplaceVariables}. FilterOptions: {FilterOptions}", fbMarketplaceVariables.ToJson(), filterOptions);
                    return new List<Car>();
                }
            ...
            {
                Logger.LogError(ex, "FbMarketplace scraper has failed. fbMarketplaceVariables: {fbMarketplaceVariables}. FilterOptions: {FilterOptions}", fbMarketplaceVariables.ToJson(), filterOptions);
                return new List<Car>();
            }
        }
        ...
        public async Task GetMakeModel(Dictionary<string, (string, List<(string, string)>)> allMakeModel)
        {
            ...
            if (makes == null)
                throw new Exception("They're no makes");
        ...
        private async Task<FbMarketplaceRequest> GetFbMarketplaceKeys()
        {
            ...
            {
                Logger.LogError(ex, "Facebook marketplace has failed to get the keys");
                return null;
            }
        }
