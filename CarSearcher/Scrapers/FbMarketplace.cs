...
namespace CarSearcher.Scrapers
{
    ...
    public partial class FbMarketplace
    {
        ...
        public FbMarketplace(HttpClient httpclient, CarLookup carlookup, CarSearcherConfig carsearcherconfig, ILogger<FbMarketplace> logger)
        {
            ...
            {
                string cookie = File.ReadAllText(CarSearcherConfig.GetFacebookCookiePath);
                MarketplaceCookie = JsonConvert.DeserializeObject<FbMarketplaceCookie>(cookie);
            }
            else
                throw new FbMarketplaceException("The website cannot startup, missing Facebook Marketplace cookie");

            // load the X_ASBD_ID (generated by your browser)
            if (File.Exists(CarSearcherConfig.GetFacebookX_ASBD_IDPath))
            {
                X_ASBD_ID = File.ReadAllText(CarSearcherConfig.GetFacebookX_ASBD_IDPath).Trim();
            }
            else
                throw new FbMarketplaceException("The website cannot startup, missing X_ASBD_ID");
        ...
        public async Task<List<Car>> ScrapeCars(FilterOptions filterOptions)
        {
            ...
            try
            {
                ...
                request.Headers.Add("Cookie", MarketplaceCookie.ToString());
                ...
                // get and set the responce cookies to keep things updated
                if (response.Headers.TryGetValues("set-cookie", out IEnumerable<string> cookies))
                {
                    Dictionary<string, string> cookieDict = CookieConvert.CookieToDictionary(cookies);

                    MarketplaceCookie.fr = cookieDict["fr"];
                    MarketplaceCookie.xs = cookieDict["xs"];

                    string marketplaceCookie = JsonConvert.SerializeObject(MarketplaceCookie, Formatting.Indented);
                    File.WriteAllText(CarSearcherConfig.GetFacebookCookiePath, marketplaceCookie);
                }
